...
Mac 全键盘训练器按 Apple 官方用户指南的章节结构,整理 macOS 系统、Finder、Terminal、文本编辑、VSCode、Chrome 六大板块共 202 条快捷键,提供三栏浏览、⌘F 跨字段搜索、⌘R 双向随机抽考和四状态进度跟踪。app 只做引导,真练回到你的 Mac;不画假 UI、不注册账号、不上传数据,所有进度保存在本机 UserDefaults。
...
| Finder | 39 | 导航、文件操作、视图模式、标签、标签页、AirDrop |
| Terminal | 31 | 窗口/标签页/拆分、导航、编辑、shell readline、搜索 |
| 文本编辑 | 28 | 光标、选择、删除、格式、剪贴板、查找 |
| VSCode | 24 | 命令面板、文件操作、编辑器、搜索、调试、Git |
| Chrome | 25 | 标签页、窗口、导航、书签、历史、下载、开发者工具 |
...
- `id` 唯一
- `appScope` 值在已知集合内
- `displayKey` 和 aliases 允许跨 app 重复 —— 同一组合键(Cmd+N、Cmd+Backspace ...)在 Finder、Terminal、文本编辑等不同 app 含义不同,要求全局唯一会拒掉合法条目
